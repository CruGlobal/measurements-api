...
module GrSync
  module Rollup
    class ChurchesRollup < ::GrSync::MeasurementRollup
      LMI = "lmi_total_gcm_nbr_of_churches"
      ...
      # Number of active churches with development > 'group_stage', that do not have church_values, grouped by
      ...
      def stats_for_churches_missing_values(period) # rubocop:disable Metrics/AbcSize
        ...
        query = base_query(period)
          ...
          .where(church[:development].gt(::Church.developments[:group_stage]))
      ...
      def stats_for_churches_with_values(period) # rubocop:disable Metrics/AbcSize
        ...
        query = base_query(period)
          ...
          .where(values[:development].gt(::Church.developments[:group_stage]))
