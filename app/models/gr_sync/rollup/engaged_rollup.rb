...
module GrSync
  module Rollup
    class EngagedRollup < ::GrSync::MeasurementRollup
      LMI = "lmi_total_build_disciples"
      ...
      # Total size of active churches with development > 'target', that do not have church_values,
      # summed by ministry in the given period
      def stats_for_churches_missing_values(period) # rubocop:disable Metrics/AbcSize
        ...
        query = base_query(period)
          .project(ministry[:gr_id], church[:size].sum)
      ...
      # Total size of active churches with church_values.development > 'target' in the given period,
      # summed by ministry
      def stats_for_churches_with_values(period) # rubocop:disable Metrics/AbcSize
        ...
        query = base_query(period)
          .project(ministry[:gr_id], values[:size].sum)
      ...
      def base_query(period) # rubocop:disable Metrics/AbcSize
        ...
        church.join(ministry).on(ministry[:id].eq(church[:ministry_id]))
