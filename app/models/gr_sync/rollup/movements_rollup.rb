...
module GrSync
  module Rollup
    class MovementsRollup < ::GrSync::MeasurementRollup
      LMI = "lmi_total_movements"
      ...
      # Number of active churches with development = 'multiplying_church', that do not have church_values, grouped by
      # ministry in the given period
      def stats_for_churches_missing_values(period) # rubocop:disable Metrics/AbcSize
        ...
        query = base_query(period)
          ...
          .where(church[:development].eq(::Church.developments[:multiplying_church]))
          .where(values[:id].eq(nil))
        execute(query.to_sql).rows.to_h
      end

      # Number of active churches with church_values.development = 'multiplying_church' in the given period
      # grouped by ministry
      def stats_for_churches_with_values(period) # rubocop:disable Metrics/AbcSize
        ...
        query = base_query(period)
          ...
          .where(values[:development].eq(::Church.developments[:multiplying_church]))
