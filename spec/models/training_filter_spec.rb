...
RSpec.describe TrainingFilter, type: :model do
  ...
  let!(:training1) do
    FactoryBot.create(:training, ministry: ministry, date: 1.week.ago)
  end

  context "filter by show tree" do
    let!(:child_ministry) { FactoryBot.create(:ministry, parent: ministry) }
    let!(:child_training) do
      FactoryBot.create(:training, ministry: child_ministry, date: 1.week.ago)
    end
  ...
  context "filter by time" do
    ...
    let!(:old_training) do
      FactoryBot.create(:training, ministry: ministry, date: 2.years.ago)
    end
